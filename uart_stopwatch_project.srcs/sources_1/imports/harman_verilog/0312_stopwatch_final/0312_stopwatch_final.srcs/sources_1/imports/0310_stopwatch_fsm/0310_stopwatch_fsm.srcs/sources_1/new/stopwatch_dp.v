...
module clk_div_100 (
    ...
    reg [$clog2(FCOUNT)-1:0] count_reg_k, count_next_k;
    reg clk_reg_k, clk_next_k; // output f/f 

    assign o_clk = clk_reg;
    assign o_clk_k = clk_reg_k;
    ...
    always @(posedge clk, posedge reset) begin
        if (reset) begin
            ...
            count_reg <= count_next;
            clk_reg <= clk_next;
            count_reg_k <= count_next_k;
            clk_reg_k <= clk_next_k;
        end
    end
    ...
    always @(*) begin
        count_next_k = count_reg_k;
        clk_next_k = 1'b0;//clk_reg;
            if (count_reg_k == (FCOUNT - 1)) begin
                count_next_k = 0;
                clk_next_k = 1'b1;
            end
            else begin
                count_next_k = count_reg_k + 1'b1;
                clk_next_k = 1'b0;
            end
    end
endmodule