...
module fnd_controller(
    ...
    assign clk_run_stop = sw[0] & clk;
    assign wire_clear = sw[1] | reset;
    decoder decoder_uut(
    ...
    counter_4 u_cnt_4(
    .clk(w_clk_10), .reset(reset), .o_sel(w_seg_sel)
);
    ...
    counter_9999 cnt_9(
    .clk(w_clk_10), .reset(reset), .o_cnt(bcd)
    );

    clk_div_10hz u_clk_div_10hz(
        .clk(clk_run_stop), .reset(wire_clear), .o_clk_10hz(w_clk_10)
    );
endmodule
...
module clk_div_10hz(
    ...
    always @(posedge clk, posedge reset) begin
        if (reset) begin
            ...
            else if (r_counter == 10_000_000/2 - 1) begin // duty radio 50%
                r_counter <= r_counter + 1'b1; 
                r_clk_10hz <= 1'b0;
            end
            else begin
